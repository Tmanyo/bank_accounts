...
minetest.register_on_joinplayer(function(player)
    if not accounts.balance[player:get_player_name()] or
        accounts.balance[player:get_player_name()] == "" or nil then
    ...
    on_rightclick = function(pos, node, player, itemstack, pointed_thing)
        ...
        if player:get_wielded_item():to_string() ~= "bank_accounts:atm_card" then
            minetest.chat_send_player(player:get_player_name(),
                                      "[ATM] Must use ATM card.")
        else
            minetest.show_formspec(player:get_player_name(),
                                   "bank_accounts:atm_home",
                                   "size[8,8]" ..
    ...
    on_metadata_inventory_put = function(pos, listname, index, stack, player)
        ...
        if listname == "ones" then
            ...
            elseif stack:get_count() == 1 then
                ...
                inv:set_stack("ones", index, nil)
                minetest.chat_send_player(player:get_player_name(),
                                          "[ATM] Must insert more than one Minegeld Note.")
            else
...
function add_deposit(player)
    if tonumber(deposited_ones) == nil then deposited_ones = 0 end
    if tonumber(deposited_fives) == nil then deposited_fives = 0 end
    if tonumber(deposited_tens) == nil then deposited_tens = 0 end
    accounts.balance[player:get_player_name()] =
        accounts.balance[player:get_player_name()] + tonumber(deposited_ones) +
            tonumber(deposited_fives) + tonumber(deposited_tens)
    save_account()
end
...
function main_form(player)
    minetest.show_formspec(player:get_player_name(),
                           "bank_accounts:atm_options",
                           "size[8,8]" .. "button[1,.5;2,1;withdrawal;Withdraw]" ..
                               "button[1,1.5;2,1;deposit;Deposit]" ..
                               "button[1,2.5;3,1;monthly_credit;Pay Monthly Credit]" ..
                               "label[4,1;Account Balance: $" ..
                               get_balance(player) .. "]" ..
                               "label[4,1.5;Total Credit Debt: $" ..
                               get_credit(player) .. "]" ..
                               "label[4,2;Monthly Credit Payment: $" ..
                               monthly_credit(player) .. "]" ..
    ...
    function(player, formname, fields)
        ...
        if formname == "bank_accounts:atm_home" then
            if fields.enter then
                ...
                for k, v in pairs(accounts.pin) do
                    if fields.fourdigitpin ==
                        accounts.pin[player:get_player_name()] then
                        main_form(player)
                    end
                end
        ...
        if formname == "bank_accounts:atm_options" then
            ...
            if fields.deposit then
                ...
                inv:set_size("tens", 1)
                local list_name = "nodemeta:" .. pos.x .. "," .. pos.y .. "," ..
                                      pos.z
                minetest.show_formspec(player:get_player_name(),
                                       "bank_accounts:deposit",
                                       "size[8,8]" .. "label[1,.5;$1]" ..
                                           "label[2,.5;$5]" .. "label[3,.5;$10]" ..
                                           "list[" .. list_name ..
                                           ";ones;.75,1;1,1]" .. "list[" ..
                                           list_name .. ";fives;1.75,1;1,1]" ..
                                           "list[" .. list_name ..
                                           ";tens;2.75,1;1,1]" ..
            ...
            if fields.monthly_credit then
                if get_credit(player) == 0 then
                    minetest.chat_send_player(player:get_player_name(),
                                              "[ATM] You do not have any credit debt.")
                else
                    minetest.show_formspec(player:get_player_name(),
                                           "bank_accounts:monthly_credit_payment",
                                           "size[8,8]" ..
            ...
            if fields.credit_card then
                ...
                inv:add_item("main",
                             {name = "bank_accounts:credit_card", count = 1})
            end
            if fields.debit_card then
                local inv = player:get_inventory()
                inv:add_item("main",
                             {name = "bank_accounts:debit_card", count = 1})
            end
        end
        if formname == "bank_accounts:withdrawal" then
            if fields.enter then
                if accounts.balance[player:get_player_name()] == nil then
                    ...
                    if fields.money == nil or fields.money == "" then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Account balance unchanged.")
                    elseif string.match(fields.money, "%a") or
                        string.match(fields.money, "%a+") then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Account balance unchanged.")
                        return false
                    elseif tonumber(accounts.balance[player:get_player_name()]) <
                        tonumber(fields.money) then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Insufficient funds.")
                    elseif tonumber(accounts.balance[player:get_player_name()]) >=
                        tonumber(fields.money) then
                        accounts.balance[player:get_player_name()] = tonumber(
                                                                         accounts.balance[player:get_player_name()]) -
                                                                         tonumber(
                                                                             fields.money)
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Funds Successfully Withdrawn!")
                        local total_tens = {}
                        local total_fives = {}
                        local total_ones = {}
                        total_tens = math.floor(tonumber(fields.money) / 10)
                        print(total_tens)
                        total_fives = math.floor(
                                          (tonumber(fields.money) -
                                              (total_tens * 10)) / 5)
                        print(total_fives)
                        total_ones = tonumber(fields.money) -
                                         ((total_fives * 5) + (total_tens * 10))
                        print(total_ones)
                        local pos = pos_info
                        local meta = minetest.get_meta(pos)
                        local inv = meta:get_inventory()
                        inv:set_size("withdrawal", 3 * 1)
                        inv:add_item("withdrawal", {
            ...
            if fields.exit then main_form(player) end
        end
        ...
        if formname == "bank_accounts:monthly_credit_payment" then
            if fields.month_button then
                if accounts.balance[player:get_player_name()] == nil then
                    ...
                    if accounts.balance[player:get_player_name()] >=
                        (get_credit(player) * .04) then
                        accounts.credit[player:get_player_name()] =
                            accounts.credit[player:get_player_name()] -
                                math.floor(get_credit(player) * .04)
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Monthly credit payment successfully paid.")
                        accounts.balance[player:get_player_name()] =
                            accounts.balance[player:get_player_name()] -
                                math.floor(get_credit(player) * .04)
                        save_account()
                    elseif accounts.balance[player:get_player_name()] <
                        (get_credit(player) * .04) then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Insufficient funds.")
                    end
            ...
            if fields.enter then
                if accounts.balance[player:get_player_name()] == nil then
                    ...
                    if string.match(fields.number, "%a") or
                        string.match(fields.number, "%a+") then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] Account balance and credit unchanged.")
                    elseif fields.number == "" or fields.number == nil then
                        minetest.chat_send_player(player:get_player_name(),
                                                  "[ATM] No amount input.")
                    else
                        if tonumber(fields.number) >
                            accounts.credit[player:get_player_name()] then
                            minetest.chat_send_player(player:get_player_name(),
                                                      "[ATM] You don't have that much credit debt.")
                        else
                            if tonumber(fields.number) >=
                                math.floor(get_credit(player) * .04) then
                                if accounts.balance[player:get_player_name()] >=
                                    tonumber(fields.number) then
                                    accounts.credit[player:get_player_name()] =
                                        accounts.credit[player:get_player_name()] -
                                            tonumber(fields.number)
                                    minetest.chat_send_player(
                                        player:get_player_name(), "[ATM] $" ..
                                            tonumber(fields.number) ..
                                            " of credit debt successfully paid.")
                                    accounts.balance[player:get_player_name()] =
                                        accounts.balance[player:get_player_name()] -
                                            tonumber(fields.number)
                                    save_account()
                                ...
                                main_form(player)
                            elseif tonumber(fields.number) <
                                math.floor(get_credit(player) * .04) then
                                minetest.chat_send_player(
                                    player:get_player_name(),
                                    "[ATM] Payment must be larger than the minimum monthly payment.")
                            elseif string.match(fields.number, "%a") or
                                string.match(fields.number, "%a+") then
                                minetest.chat_send_player(
                                    player:get_player_name(),
                                    "[ATM] Account balance and credit unchanged.")
...
-- savings interest
local timer = 0
interest_period = tonumber(
                      minetest.settings:get("bank_accounts.interest_period")) or
                      5 -- 720
interest_rate =
    tonumber(minetest.settings:get("bank_accounts.interest_rate")) or 1 -- 1 percent

minetest.register_globalstep(function(dtime)
    timer = timer + dtime;
    if timer >= interest_period then
        timer = 0
        -- get all players
        for name, value in minetest.get_auth_handler().iterate() do
            -- minetest.log("info", "[Currency] interest for "..name)
            minetest.log("info", "[Bank] interest for " .. name)
            accounts.balance[name] = math.floor(
                                         accounts.balance[name] *
                                             (1 + interest_rate / 100) + 0.5)
            save_account()
        end
    end
end)

-- If you have server priv, you view/change interest_rate.
minetest.register_chatcommand("interest_period", {
    params = "<number>",
    description = "View/Change Interest Period.",
    func = function(name, params)
        local s = params
        local number = s:match("%d+")
        if not s:match("%d") then
            minetest.chat_send_player(name, "[Bank] Interest Period is " ..
                                          interest_period)
        else
            if minetest.check_player_privs(name, {server = true}) == true then
                interest_period = number
                minetest.chat_send_player(name,
                                          "[Bank] Interest Period is changed to " ..
                                              interest_period)
            end
        end
    end
...
-- If you have server priv, you view/change interest_rate.
minetest.register_chatcommand("interest_rate", {
    params = "<number>",
    description = "View/Change Interest Rate.",
    func = function(name, params)
        local s = params
        local number = s:match("%d+")
        if not s:match("%d") then
            minetest.chat_send_player(name, "[Bank] Interest rate is " ..
                                          interest_rate .. "%")
        else
            if minetest.check_player_privs(name, {server = true}) == true then
                interest_rate = number
                minetest.chat_send_player(name,
                                          "[Bank] Interest rate is changed to " ..
                                              interest_rate .. "%")
            end
        end
    end
